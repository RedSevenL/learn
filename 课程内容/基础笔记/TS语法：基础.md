...
2. 再给关键变量标注类型。
3. 再给函数参数和返回值标注类型。
4. 如果对象结构重复出现，再提取 `interface` 或 `type`。
5. 最后根据 TS 报错继续调整类型。
...
这个顺序是有道理的，尤其适合刚开始练习 TS：

- 先写主体，可以避免一上来被类型语法卡住。
- 后补类型，可以更清楚地理解“这个值到底是什么”。
- TS 报错会反过来提醒代码结构哪里不明确。

但也要注意：函数参数、接口返回数据、DOM 元素这些“边界位置”，最好尽早标类型。因为边界位置最容易出错，类型能帮你提前发现问题。
...
- 回调函数也需要指定函数签名。
- `type` 可以给复杂类型起别名。
- 入门练习可以先写主体逻辑，再逐步补充类型；项目里的函数边界和核心数据结构要尽早标类型。
